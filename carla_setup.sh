...
sudo wget https://carla-releases.s3.us-east-005.backblazeb2.com/Linux/CARLA_0.9.15.tar.gz -P $BASE_PATH
sudo mkdir -p $BASE_PATH/CARLA_LATEST
echo "Extracting Carla to Desktop/CARLA_LATEST/"
sudo tar -xvf $BASE_PATH/CARLA_0.9.15.tar.gz -C $BASE_PATH/CARLA_LATEST > /dev/null
sudo rm $BASE_PATH/CARLA_0.9.15.tar.gz
...
echo "Remember to activate your conda environment using 'conda activate carla' every time you reopen a new console tab"
echo "To run CARLA, navigate to the directory of installation and run './CarlaUE4.sh -carla-rpc-port=[port-number here]'"
